...
- 颜色模式：浅色 / 暗色 / 跟随系统
- 自定义背景颜色（浅色、暗色分别设置）
- **自定义背景媒体**：用 PNG 图片、GIF 动图或 MP4 视频作为启动遮罩背景
  - 内置裁切器：拖拽平移、双指缩放、90° 旋转
  - 不透明度可调；GIF / MP4 循环播放
  - （GIF / MP4 采用「原样存储 + 裁切变换」方案，保留动画与视频播放）

### 动画

- 退出动画效果：默认（Android 原生 ripple）/ 粒子扩散 / 粒子消散（实验性）
- 粒子参数：运动时长、持续时长等
...
- 还原 AOSP 应用过渡动画（关闭 ColorOS 的连续转场覆盖）

### 应用配置

- 显示桌面图标
- 应用内语言切换（跟随系统 / 简体中文 / English）
- 重启菜单（重启系统界面 / 重启系统），所有页面顶栏可用
