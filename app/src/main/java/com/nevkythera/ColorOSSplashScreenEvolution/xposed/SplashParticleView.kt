...
/**
 * Telegram「删除消息」尘埃粒子消散效果的原生移植。
 *
 * ────────────────────────────────────────────────────────────────────────
 * 设计（本轮重写）
 * ────────────────────────────────────────────────────────────────────────
 *  1. **不画底图**：整块 splash 从第 0 帧起就完全由尘埃粒子构成，
 *     不存在「一部分粒子、一部分淡出」。
 *  2. **整屏都有粒子**：粒子覆盖整个画面（含下半屏）。
 *  3. **渐进消散**：每颗粒子按所属列分配寿命（左列短、右列长）→
 *     整屏一起变尘埃，再从左到右逐列消失。
 *
 * ────────────────────────────────────────────────────────────────────────
 * 渲染方式（重要）
 * ────────────────────────────────────────────────────────────────────────
 *  ★★ 使用**逐粒子 `Canvas.drawCircle`**。
 *  曾经为了性能改成「按 (颜色×alpha×半径) 分桶 + `Canvas.drawPoints`」，
 *  但真机上**完全画不出东西**（id46 起每一版都看不到粒子），
 *  因此回退到「逐粒子绘制」这条**已验证可渲染**的路径。
 *  粒子数用 [MAX_PARTICLES] 限制以控制开销（本效果只持续几百毫秒）。
 *
 * 官方公式与随机参数（取自 Aghajari/ThanosEffect 公开的 README 与源码）：
 *  - 网格步长 `perPx`（默认 6px）、只取 alpha>10 的像素；半径随机 ∈ [1.5, 6] px；
 *  - 速度 4~8dp；`tX=±(96dp+rnd(56dp))`、`tY=±(32dp+rnd(56dp))`（为负则 ÷4）；
 *  - 初始 alpha ∈ {0.3, 0.6, 1.0}；每帧（f = 已存活时间 / 寿命）：
 *      y = y0 + (y0-cy)/cy·tY·f − (f·vY)²
 *      x = x0 + (x0-cx)/cx·tX·f
 *      r = max(0.5, r0·(1−f))
 *      alpha = α0·min(1.2−f, 1)
 *  ★ 发布到 GitHub 前需在 README 的「致谢/参考」注明来源与许可证。
 *
 * ⚠️ 不使用 GL / SurfaceView（起始窗口里 Surface 建不起来，已实测失败）。
 *
 * @param source     当前 splash 截图（**仅用于采样颜色**，采样后立即回收）
 * @param durationMs 动画总时长（毫秒，200~2000）
 * @param onFinish   动画结束后回调（只会调用一次）
 */
